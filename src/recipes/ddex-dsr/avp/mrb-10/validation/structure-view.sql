use schema %ENV%_DDEX_DSR.VALIDATION_AVP_MRB_10;
...
block_structure_pre1 as (
  ...
  array_to_string(block_structure, '') regexp '(RE03)(AS03)+(SU03.01)+' pass
...
header_summary_structure_pre1 as (
  select asset_id, block_id, block_structure,
  array_to_string(block_structure, '') regexp 'HEAD(SY04.01)+FOOT' pass
  from block_structure_pre1
